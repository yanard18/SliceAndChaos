...
public class LiquidShaderGUI : ShaderGUI
{
    ...
    private void DrawWaveSettings(MaterialProperty[] properties)
    {
        ...
        string s = WaveSettingsOpen ? "<size=20>Wave & Flow Settings ▲</size>" : "<size=20>Wave & Flow Settings ▼</size>";
        EditorGUILayout.Separator();
        ...
        if (WaveSettingsOpen)
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("Wave Frequency<size=12><color=#fff>(⟷)</color></size>",
                "The frequency of the waves. Increased values mean you get more waves mixing in together. On high numbers, some repetitions may be noticed."), MainStyle, options);
    ...
    {
        EditorGUILayout.Separator();
        string s = EdgeSettingsOpen ? "<size=20>Edge Settings ▲</size>" : "<size=20>Edge Settings Settings ▼</size>";
        EditorGUILayout.Separator();
        if (GUILayout.Button(s, TitleStyle, GUILayout.ExpandWidth(true)))
        {
        ...
        if (EdgeSettingsOpen)
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("Edge Scale<size=12><color=#fff>(⟷)</color></size>",
                "Scale (on Y axis) of the wave. Basically makes the waves bigger or smaller. Vertically"), MainStyle, options);
    ...
    {
        EditorGUILayout.Separator();
        string s = LightSettingsOpen ? "<size=20>Light & Glow Settings ▲</size>" : "<size=20>Light & Glow Settings ▼</size>";
        EditorGUILayout.Separator();
        if (GUILayout.Button(s, TitleStyle, GUILayout.ExpandWidth(true)))
        {
        ...
        if (LightSettingsOpen)
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("Light Texture",
                "Texture to apply as a light source for a more 3D effect. Usually just a centered white radial gradient is enough."), MainStyle, options);
            GUILayout.FlexibleSpace();
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Light Position", "Position of the light texture. Wraps around."), MainStyle, options);
            properties[13].vectorValue = EditorGUILayout.Vector2Field("", properties[13].vectorValue, options);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Light Tiling", "Tiling to adjust the scale of the light texture."), MainStyle, options);
    ...
    {
        EditorGUILayout.Separator();
        string s = CausticsSettingsOpen ? "<size=20>Caustics Settings ▲</size>" : "<size=20>Caustics Settings Settings ▼</size>";
        EditorGUILayout.Separator();
        if (GUILayout.Button(s, TitleStyle, GUILayout.ExpandWidth(true)))
        {
        ...
        if (CausticsSettingsOpen)
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("Caustics Color", 
                "Color of the caustics. Usually just white is fine but can go different. Go HDR to make it glow under bloom."), MainStyle, options);
            ...
            EditorGUILayout.LabelField(new GUIContent("Lens Distortion Texture",
                "Texture for applying a lens distortion. Whites are distorted, alphas are not."), MainStyle, options);
            GUILayout.FlexibleSpace();
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Lens Distortion Amount<size=12><color=#fff>(⟷)</color></size>", 
    ...
    private void DrawMainSettings(MaterialProperty[] properties)
    {
        ...
        if (MainSettingsOpen)
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("Contrast<size=12><color=#fff>(⟷)</color></size>",
                "Overall contrast. Can go negative or very high but will produce unexpected results at those numbers. Ideally around 0.5 to 2"), MainStyle, options);
            ...
            EditorGUILayout.LabelField(new GUIContent("Random Seed<size=12><color=#fff>(⟷)</color></size>",
                "Random seed for various calculations. Advised to change if you're copying a material to create another liquid."), MainStyle, options);
            ...
            EditorGUILayout.LabelField(new GUIContent("Alpha Mask",
                "Mask to use. Use this to fit the liquid inside an image like a potion or sphere. Check the examples."), MainStyle, options);
            GUILayout.FlexibleSpace();
            ...
            EditorGUILayout.LabelField(new GUIContent("Transparency<size=12><color=#fff>(⟷)</color></size>",
                ...
                " this setting can go higher than 1 to produce overflowing results. Ideal range is 0-2 but can go higher."), MainStyle, options);
